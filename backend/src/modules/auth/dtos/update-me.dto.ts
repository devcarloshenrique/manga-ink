...
const updateMeBodySchema = z.object({
  ...
  currentPassword: z.string().min(6, 'Senha atual deve ter no mínimo 6 caracteres').optional(),
  password: z.string().min(6, 'Nova senha deve ter no mínimo 6 caracteres').optional(),
  confirmPassword: z.string().min(6, 'Confirmação de senha deve ter no mínimo 6 caracteres').optional(),
})
